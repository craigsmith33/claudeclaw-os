...
export function buildDashboardApp(botApi?: Api<RawApi>): Hono {
  ...
  app.use('*', async (c, next) => {
    ...
    if (origin) {
      // A valid token in the query string proves the request came from the
      // dashboard (the token is only embedded in URLs generated by the bot).
      // Telegram's in-app browser and some WebViews send an Origin that doesn't
      // match the host, so we skip the CSRF check when the token is valid.
      const reqToken = c.req.query('token');
      const hasValidToken = !!(reqToken && DASHBOARD_TOKEN && reqToken === DASHBOARD_TOKEN);
      if (!hasValidToken) {
        let host = '';
